...
struct Symbol {
  const char *name; // symbol's name. The symbol doesn't own this pointer, but
                    // it is allocated somewhere else (ex: in Token)
  SymKind kind;
  Type type;
  ...
  union {       // specific data fo each kind of symbol
    ...
    struct {
      ...
      Symbol *locals; // all local vars of a function, including the ones from
                      // its inner domains
